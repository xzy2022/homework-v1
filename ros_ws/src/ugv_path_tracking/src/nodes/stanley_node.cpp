...
class StanleyNode {
 public:
  StanleyNode()
    ...
    controller_ = std::make_unique<ugv::path_tracking::StanleyController>();
    ...
    private_nh_.param("command_topic", command_topic_, std::string("/vehicle/stanley_cmd"));
    private_nh_.param("control_period", control_period_, control_period_);

    if (!controller_->Configure(private_nh_)) {
      ROS_WARN("[StanleyNode] Failed to configure controller with provided parameters");
    }

    command_pub_ = nh_.advertise<ugv_msgs::ControlCommand>(command_topic_, 10);
    trajectory_sub_ =
        nh_.subscribe(trajectory_topic_, 1, &StanleyNode::OnTrajectory, this);
    state_sub_ = nh_.subscribe(vehicle_state_topic_, 1, &StanleyNode::OnState, this);
    timer_ = nh_.createTimer(ros::Duration(control_period_), &StanleyNode::OnTimer, this);
  }
 ...
 private:
  ...
  std::unique_ptr<ugv::path_tracking::StanleyController> controller_;
